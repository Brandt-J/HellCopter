...
func set_node_to_follow(node_to_follow: Node3D) -> void:
	...
	_camera_3d.global_position = _node_to_follow.to_global(Vector3(0, camera_height, camera_distance))
...
func _process(delta: float):
	if is_instance_valid(_node_to_follow):
		...
		_camera_3d.global_position = lerp(_camera_3d.global_position, pos, 0.98*delta)
